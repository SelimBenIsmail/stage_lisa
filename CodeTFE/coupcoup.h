...
struct configuration{ 
  ...
  uint16_t expTime ; // Temps d'attente avant le déclanchement de l'appareil photo
...
// ----- New class MyStepper, Accelstepper daugther -----
class MyStepper: public AccelStepper { 
  ...
  long angleToStep (float angleValue){ // Configuré sur un quart step + reducteur 1:100 
...
void setup(){
  ...
  loadEEPROM();
  // ----- Init Timer -----
  Timer1.initialize(500000); // Initialise le Timer1  avec une période d'une demi seconde
  Timer1.attachInterrupt(Tim1); // Attaches Tim1() à l'interruption lié au debordement du Timer1
} // End of steup
