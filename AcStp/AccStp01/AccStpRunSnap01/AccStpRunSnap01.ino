...
//------------------------------ fonction runSnap ----------------------------------------------------------- 
  void runSnap(int angle,int expTime, int pinFocus, int pinTrig){
    setCurrentPosition(0);
    while(currentPosition() <= 6400){    
      snap(expTime, pinFocus, pinTrig);
      move(angleToStep(angle));
      runToPosition();
    }
...
void loop(){ 
  ...
  if(irrecv.decode(&resultsIR)){
    ...
    if (codeIR == "551489775"){ //Bouton off
      ...
      mem=turntable.currentPosition();
      turntable.runSnap(45,3,FOCUS,TRIG);
      turntable.setCurrentPosition(mem);
