...
class MyStepper: public AccelStepper { 
 ...
 //----------------------------------- function checkError---------------------------------------------------------
  ...
  lastState=state;
  state=98;
    if(enableMotor)error(40); //moteur désactivé
    else if(!conf.expTime)error(42); //temps d'expo à 0 
    else if(!conf.accel)error(43); //accelaration à 0
    else if(!conf.max_Speed)error(44); //max speed à 0
    else return(0);
...
void loop(){ 
  ...
  if(irrecv.decode(&resultsIR)){
    ...
    if (resultsIR.value == LG_OFF){ //Bouton off
      ...
      if(enableMotor)turntable.error(erID=40);
