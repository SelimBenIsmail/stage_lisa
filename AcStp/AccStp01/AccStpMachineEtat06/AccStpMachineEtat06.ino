...
const uint32_t FULL_ROT = 200*ST_MOD*RP; // Rotation complète: 80000step
...
{ 
  ...
  long stepToAngle (long stepValue) { //configuré sur un quart step + reducteur 1:100  
    long angleValue = (stepValue%(FULL_ROT)) *(1.8 /(ST_MOD*RP));
    return (angleValue);
  }// end function stepToAngle

  //------------------------------ function angleToStep -----------------------------------------------------------
  long angleToStep (long angleValue){ //configuré sur quart step + reducteur 1:100 
  ...
  switch (state){
    ...
    case 6: // RunSnap
      if(irrecv.decode(&resultsIR)){
        ...
        if (resultsIR.value == LG_EXIT){ //Bouton EXIT
          Serial.println("EXIT");
          turntable.setCurrentPosition(FULL_ROT);
        }//end if result
      }//end if decode
      lcd.clear();
      lcd.print("Progress: ");
