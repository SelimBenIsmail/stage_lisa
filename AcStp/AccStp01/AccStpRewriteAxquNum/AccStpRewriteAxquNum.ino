...
void loop(){ 
  ...
  if(irrecv.decode(&resultsIR)){
    ...
    if (resultsIR.value == LG_OFF){ //Bouton off
      ...
      config_expTime = turntable.acqNum(3,config_expTime); 
    }
    else if(resultsIR.value == LG_MENU){ // Bouton Q.Menu
      lcd.clear();
      lcd.print("Accel : ");
      lcd.print(config_accel);   
      turntable.setAcceleration(config_accel=turntable.acqNum(3,config_accel)); 
    } 
    else if(resultsIR.value == LG_INFO){ // Bouton Info
      lcd.clear();
      lcd.print("Max Speed : ");
      lcd.print(config_maxSpeed);
      config_maxSpeed = turntable.acqNum(4,config_maxSpeed);  
      turntable.setMaxSpeed(config_maxSpeed); 
    }  
      ...
      config_interv = turntable.acqNum(3,config_interv);
      lcd.clear();
      ...
      turntable.moveTo(turntable.angleToStep(float(turntable.acqNum(3,turntable.stepToAngle(turntable.currentPosition()))%360)));
